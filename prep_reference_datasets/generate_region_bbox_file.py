...
try:
    ...
    logging.info(f"Error: Required package not found: {e}")
    logging.info("Please install required packages:")
    logging.info("pip install geopandas pandas shapely")
    sys.exit(1)
        ...
        if max_gap > 180 or wrap_gap < current_span:
            ...
            if max_gap > wrap_gap:
                ...
                if lon_max < lon_min:
                    ...
                    logging.info(f"    Warning: Geometry appears to cross dateline, bbox may be wide")
                    lon_min, lon_max = min(lons), max(lons)
            else:
...
def process_gpkg_to_csv(gpkg_path, csv_path):
    ...
    try:
        # Read the GeoPackage
        logging.info(f"Reading GeoPackage: {gpkg_path}")
        ...
        logging.info(f"Found {len(gdf)} features with {len(gdf['SUBREGION'].unique())} unique subregions")
        ...
        for subregion in gdf['SUBREGION'].unique():
            ...
            if lon_min is not None:  # Valid bbox computed
                ...
                logging.info(f"  {subregion}: [{lon_min:.6f}, {lat_min:.6f}, {lon_max:.6f}, {lat_max:.6f}]")
            else:
                logging.info(f"  Warning: Could not compute bbox for {subregion}")
        ...
        if results:
            ...
            logging.info(f"Processed {len(results)} regions")
        else:
            raise ValueError("No valid bounding boxes could be computed")
            
    except Exception as e:
        logging.info(f"Error processing GeoPackage: {e}")
        sys.exit(1)
    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                ...
                logging.info(f"Warning: Could not create geometry for {row['name']}: {e}")
        ...
        logging.info(f"Preview GeoPackage generated: {preview_path}")
        logging.info(f"Contains {len(gdf)} bounding box rectangles")
        
    except Exception as e:
        logging.info(f"Error generating preview: {e}")
        sys.exit(1)
...
def main():
    ...
    if not args.generate_preview:
        if not os.path.exists(args.path_gpkg):
            logging.info(f"Error: GeoPackage file not found: {args.path_gpkg}")
            sys.exit(1)
        
        if not args.path_gpkg.lower().endswith('.gpkg'):
            logging.info("Warning: Input file doesn't have .gpkg extension")
    ...
    if args.generate_preview:
        ...
        if not os.path.exists(args.path_csv_out):
            logging.info(f"Error: CSV file not found: {args.path_csv_out}")
            sys.exit(1)
