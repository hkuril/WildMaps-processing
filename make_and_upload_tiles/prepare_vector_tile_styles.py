...
def process_style_file(input_path):
    ...
    logging.info(f"Filled style saved to: {filled_path}")
    ...
    logging.info(f"Overlay style saved to: {overlay_path}")

    with open(underlay_path, "w", encoding="utf-8") as f:
        json.dump(underlay_style, f, indent=2)
    logging.info(f"Underlay style saved to: {underlay_path}")

if __name__ == "__main__":
    if len(sys.argv) != 2:
        logging.info("Usage: python split_style.py <path/to/style.json>")
        sys.exit(1)
